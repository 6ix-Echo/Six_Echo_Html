...
六叔的英文名是 **Six-Echo**，项目中的文件命名、Skill 名称和公开说明统一使用 `six-echo`，不再使用 `liu-shu` 作为拼音缩写。

个人 IP 的固定识别元素：红色花纹帽、天狗面具、不露脸、自然身材比例、肌肉线条、克制但有力量的姿态。头像金句保持原文，只优化构图和排版。
...
- 技术感但不冰冷
- 真实、克制、好奇、可执行
- 一眼能看出是 Six-Echo，而不是通用 AI 模板

## 禁忌与注意事项

- 不使用 `liu-shu` 作为新的文件名或公开资源名，统一使用 `six-echo`。
...
- [ ] Six-Echo 文件命名统一，没有新产生的 `liu-shu` 资源引用
- [ ] 背景、色条、标题和卡片层级统一
- [ ] 标题对比度足够，正文不会使用过浅灰色
